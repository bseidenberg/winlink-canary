...
def render_html(health, failed_urls, refresh_interval):
    ...
    html = [
        ...
        "  const w = window.open('', 'Polling History', features);",
        "  w.document.write(`<!DOCTYPE html><html><head><title>Polling History</title>` +",
        "  `<style>body { font-family: Arial, sans-serif; margin: 15px; } h3 { margin-top: 0; }</style>` +",
        "  `</head><body><h3>` + name + `</h3><pre>` + hist + `</pre></body></html>`);",
        "  w.document.close();",
        "  w.focus();",
        "}",
        "</script>",
